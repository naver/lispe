...
(class@ Model (model_path config tok init)
    ...
    (defun generate(prompt max_tokens)
        ...
        (setq generated_tokens (clone prompt_tokens))
        (setq current_input (torch_unsqueeze (torch_to_mps (tensor_create (clone prompt_tokens))) 0))
        (setq generated ())
        ...
        (setq time_with_cache 
            (elapse 
                (setq generated_tokens 
                    (torch_hf_generate model_id prompt_tokens (tok (eos)) size_sequence options)
                )
            )
        )
