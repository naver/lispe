...
public:
    ...
    Element* _eval(u_ustring&);
    Element* _eval(wstring& w) {
        u_ustring s = _w_to_u(w);
        return eval(s);
    }

    
    Element* _eval(string w) {
        u_ustring s;
        s_utf8_to_unicode(s, w, w.size());
        return eval(s);
    }
...
inline bool sendError() {
   {
      delegation->set_error(delegation->_THEEND);
      return;
   }
}
...
inline bool unboundAtomError(int16_t label) {
   ...
   err += U"'";
   {
      delegation->set_error(new Error(err));
      return;
   }
}
...
inline Element* recordingMethod(Element* e, int16_t label) {
   ...
   for (long i = 0; i < parameters->size(); i++) {
      ...
      if (dico != NULL) {
         if (!dico->verify())
            return new Error("Error: When building a pattern for a dictionary, keys with actual values (number of string) should appear first");
...
void create_name_space(int16_t label) {
   if (label < l_final && label != v_mainspace)
      {
      delegation->set_error(new Error("Error: Cannot use this label to define a space"));
      return;
   }
   ...
   if (delegation->namespaces.check(label))
      ...
      if (delegation->function_pool.size() > 65534)
         {
         delegation->set_error(new Error("Error: Maximum number of namespace reached"));
         return;
      }
