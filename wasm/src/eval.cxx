...
Element* List::evalthreadspace(LispE* lisp, long listsize, long i) {
   ...
   for (; i < listsize && thrown_error == NULL && element->type != l_return; i++) {
      element->release();
...
Element* List::eval_pattern(LispE* lisp, int16_t function_label) {
      ...
      if (ilabel > l_final && element->type != t_data) {
         ...
         if (match != check_ok) {
            ...
            if (match == check_mismatch)
               ...
               arguments->release();
               lisp->resetStack();
               return lisp->delegation->set_error(new Error(L"Error: Size mismatch between argument list and data structure definition"));
            }
            else {
               ...
               message << " (" << lisp->asString(lisp->getDataStructure(ilabel)->index(match)->label()) << " required)";
               {
                  arguments->release();
                  lisp->resetStack();
                  return lisp->delegation->set_error(new Error(message.str()));
               }
            }
         }
      }
   ...
   if (nbarguments == 4)
      ...
      element = null_;
      for (i = 3; i < nbarguments && thrown_error == NULL && element->type != l_return; i++) {
         element->release();
...
void List::evalthread(LispE* lisp, List* body) {
   ...
   while (element == terminal_) {
      if (nbarguments == 4)
         ...
         element = null_;
         for (long i = 3; i < nbarguments && thrown_error == NULL && element != terminal_ && element->type != l_return && !element->isError(); i++) {
            element->release();
...
void List::evalthread(LispE* lisp, List* body) {
   ...
   while (element == terminal_) {
      if (nbarguments == 4)
         ...
         element = null_;
         for (long i = 3; i < nbarguments && thrown_error == NULL && element != terminal_ && element->type != l_return; i++) {
            element->release();
            element = body->liste[i]->eval(lisp);
         }
      }
      ...
      if (element->isList()) {
         if (element->index(0) == emptylist_) {
            ...
            if (element->size() == 2) {
               ...
               if (l != NULL)
                  l->release();
               {lisp->delegation->set_error(new Error(L"Error: Wrong parameter description"));return;}
            }
         }
         else {
            label = element->size();
...
Element* List::eval_thread(LispE* lisp, List* body) {
   ...
   if (tid == NULL) {
      delete thread_lisp;
      return new Error("Error: Too many threads created. Cannot execute it...");
   }
   return True_;
}
...
Element* List::eval_lambda(LispE* lisp, List* body) {
   ...
   do {
      if (nbarguments == 3)
         ...
         element = null_;
         for (i = 2; i < nbarguments && thrown_error == NULL && element != terminal_ && element->type != l_return; i++) {
            element->release();
            element = body->liste[i]->eval(lisp);
         }
      }
...
Element* Atome::_eval(LispE* lisp) {
...
//------------------------------------------------------------------------------
Element* Instruction::_eval(LispE* lisp) {
...
#endif
Element* LispE::_eval(u_ustring& code) {
...
Element* List::_eval(LispE* lisp) {

   Element* e = (this->*lisp->delegation->evals[lisp->checkState(this, liste.size())])(lisp);
   if (thrown_error) {
      return lisp->check_error(this, thrown_error, -1, 0);
   }
   lisp->resetStack();
   return e;
}
...
Element* Listincode::_eval(LispE* lisp) {
...
Element* List_execute::_eval(LispE* lisp) {
...
Element* Listreturn::_eval(LispE* lisp) {
    return lisp->provideReturn();
}

Element* Listreturnelement::_eval(LispE* lisp) {
...
Element* List::evall_root(LispE* lisp) {
   ...
   for (size_t i = 1; i < listsize && thrown_error == NULL && element->type != l_return && thrown_error == NULL; i++) {
      element->release();
      element = liste[i]->eval(lisp);
   }
...
Element* List::evall_root(LispE* lisp) {
   ...
   for (size_t i = 1; i < listsize && thrown_error == NULL && element->type != l_return; i++) {
      element->release();
      element = liste[i]->eval(lisp);
   }
...
Element* List::reduce_with_list(LispE* lisp, Element* l1, Element* op, long sz) {
...
Element* List::reduce_lambda(LispE* lisp, Element* l1, Element* op, long sz) {
   ...
   element = op->eval_lambda_min(lisp);
   if (element->type != l_return) {
...
Element* List::backreduce_with_list(LispE* lisp, Element* l1, Element* op, long sz) {
   bool sb = lisp->set_true_as_one();
...
Element* List::backreduce_lambda(LispE* lisp, Element* l1, Element* op, long sz) {
   if (op->index(1)->size() != 2)
      return new Error("Error: Wrong number of arguments");
...
Element* List::scan_with_list(LispE* lisp, Element* l1, Element* op, long sz) {
   bool sb = lisp->set_true_as_one();
...
Element* List::scan_lambda(LispE* lisp, Element* l1, Element* op, long sz) {
   if (op->index(1)->size() != 2)
      return new Error("Error: Wrong number of arguments");
...
Element* List::backscan_with_list(LispE* lisp, Element* l1, Element* op, long sz) {
   bool sb = lisp->set_true_as_one();
...
Element* List::backscan_lambda(LispE* lisp, Element* l1, Element* op, long sz) {
   if (op->index(1)->size() != 2)
      return new Error("Error: Wrong number of arguments");
...
Element* List::evall_data(LispE* lisp) {
   ...
   for (; i < listsize && thrown_error == NULL; i++) {
      second_element = liste[i];
      if (!second_element->isList() || !second_element->size())
         {
         second_element->release();
         return lisp->delegation->set_error(new Error(L"Error: A data structure can only contain non empty lists"));
      }
      lab = second_element->index(0)->label();
      if (lab == v_null)
         {
         second_element->release();
         return lisp->delegation->set_error(new Error(L"Error: Missing definition name in data structure"));
      }
...
Element* List::evall_deflib(LispE* lisp) {
   ...
   if (label == v_null)
      return new Error(L"Error: Missing name in the declaration of a function");
   if (!liste[2]->isList())
      return new Error(L"Error: List of missing parameters in a function declaration");
...
Element* List::evall_defspace(LispE* lisp) {
   int16_t label = liste[1]->label();
   if (label == l_thread)
      return new Error("Error: 'thread' is a reserved space name");
...
Element* Listincode::eval_infix(LispE* lisp) {
   long listsize = liste.size();
   if (listsize % 2)
      return new Error("Error: Infix expression is malformed");
   ...
   while (ival < listsize) {
      ...
      for (; iop < listsize - 1; iop+=2) {
         if (!liste[iop]->isAtom())
            return new Error("Error: Expecting an operator in this infix expression");
         if (op != liste[iop]) {
            op = liste[iop];
            break;
         }
...
Element* List::evall_lambda(LispE* lisp) {
   if (liste.size() < 3)
      return new Error("Error: wrong number of arguments");
   if (!liste[1]->isList())
      return new Error(L"Error: Missing parameter list in a lambda declaration");
   last()->setterminal();
   return this;
}
...
Element* List::evall_switch(LispE* lisp) {
   ...
   for (i = 2; i < size(); i++) {
      if (!liste[i]->isList() || !liste[i]->size())
         return new Error("Error: wrong 'switch statement'");
...
Element* List::evall_link(LispE* lisp) {
   ...
   if (!atome->isAtom()) {
      atome->release();
      return new Error("Error: the second argument must be an atom");
   }
