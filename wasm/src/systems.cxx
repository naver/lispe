...
Element* seek(LispE* lisp, long nb, long off) {
   if (!open)
      return new Error ("Error: 'stream' is not opened");
   if (!off)
...
Element* read(LispE* lisp, long nb) {
   if (!open || mode != file_read)
      return new Error ("Error: expecting 'stream' to be in 'read' mode");
...
Element* write(LispE* lisp, Element* line) {
   if (!open || mode != file_write)
      return new Error ("Error: expecting 'stream' to be in 'write' mode");

   string ch = line->toString(lisp);
   myfile << ch;
   if (myfile.fail())
      return new Error("Error: 'write' operation did fail");
   return True_;
}
...
Element* writeln(LispE* lisp, Element* line) {
   ...
   myfile << ch << std::endl;
   if (myfile.fail())
      return new Error("Error: 'writeln' operation did fail");
   return True_;
}
...
Element* getstream(LispE* lisp) {
   ...
   if (a_stream->type != type)
      return new Error("Erreur: missing 'file' stream");
   return a_stream;
}
...
Element* _eval(LispE* lisp) {
   ...
   switch (fcommand) {
      case file_open: {
         ...
         delete str;
         return new Error("Error: could not open a file");
      }
      case file_close: {
...
Element* year(LispE* lisp) {
   ...
   if (e->type != type)
      return new Error("Error: wrong type for 'year'");
...
Element* month(LispE* lisp) {
   Element* e = lisp->get_variable(v_date);
   if (e->type != type)
      return new Error("Error: wrong type for 'month'");
   Dateitem* dt = (Dateitem*)e;
   int vl = extract_value(lisp, v_d);
...
Element* day(LispE* lisp) {
   Element* e = lisp->get_variable(v_date);
   if (e->type != type)
      return new Error("Error: wrong type for 'day'");
   Dateitem* dt = (Dateitem*)e;
   int vl = extract_value(lisp, v_d);
...
Element* hour(LispE* lisp) {
   Element* e = lisp->get_variable(v_date);
   if (e->type != type)
      return new Error("Error: wrong type for 'hour'");
   Dateitem* dt = (Dateitem*)e;
   int vl = extract_value(lisp, v_d);
...
Element* minute(LispE* lisp) {
   Element* e = lisp->get_variable(v_date);
   if (e->type != type)
      return new Error("Error: Wrong type for 'minute'");
   Dateitem* dt = (Dateitem*)e;
   int vl = extract_value(lisp, v_d);
...
Element* second(LispE* lisp) {
   Element* e = lisp->get_variable(v_date);
   if (e->type != type)
      return new Error("Error: wrong type for 'second'");
   Dateitem* dt = (Dateitem*)e;
   int vl = extract_value(lisp, v_d);
...
Element* weekday(LispE* lisp) {
   Element* e = lisp->get_variable(v_date);
   if (e->type != type)
      return new Error("Error: wrong type for 'weekday'");
   Dateitem* dt = (Dateitem*)e;
...
Element* yearday(LispE* lisp) {
   Element* e = lisp->get_variable(v_date);
   if (e->type != type)
      return new Error("Error: wrong type for 'yearday'");
   Dateitem* dt = (Dateitem*)e;
...
Element* _eval(LispE* lisp) {
   ...
   case sys_isdirectory:
      ...
      case sys_command: {
         ...
         #endif                
         if (fp == NULL)
            return new Error("Error: the pipe did not open properly");
         ...
         if (status == -1) {
            resultat->release();
            return new Error("Error: when closing the pipe");
         }
