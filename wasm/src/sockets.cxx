...
Element* methodCreateServer(LispE* lisp, int nbclients, int kport, string server_name) {
   ...
   if (hp != NULL)
      return new Error("Error: SOCKET(831): Server already launched on this socket");
   if (server_name.size() >= MAXHOSTNAME)
      return new Error("Error: SOCKET(850): Wrong server name");
...
Element* methodRead(LispE* lisp, int socketclient) {
   ...
   if (currentsock == SOCKET_ERROR)
      return new Error("Error: SOCKET(858): No client connected");
...
Element* methodReadRaw(LispE* lisp, SOCKET currentsock, int nbbytes) {
   ...
   if (server == true) {
      if (currentsock == SOCKET_ERROR)
         return new Error("Error: SOCKET(824): Non connected socket");
   }
   else {
      currentsock = sock;
   }
...
Element* methodGetpeername(LispE* lisp, int socketclient) {
   ...
   return new Error("Error: SOCKET(852): You cannot use GETPEERNAME on the client side");
}
...
Element* createSocket(LispE* lisp) {
   ...
   if (sock == SOCKET_ERROR)
      return new Error("Error: SOCKET(856): Socket error");
   #ifdef WIN32
   ...
   if (checkipaddres(servername) == false) {
      ...
      if (hp == NULL)                             /* we don't exist !? */
         return new Error("Error: SOCKET(851): Cannot find host by name on this machine");
   ...
   switch (action) {
      ...
      case sock_wait: {
         ...
         if (sock->type != type_socket_element)
            return new Error("Error: expecting a 'socket' object");
