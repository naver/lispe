   ...
   switch (e->type) {
      case t_floats: {
         ...
         if (n->liste.check(0))
            {
            release();
            return new Error("Error: division by zero");
         }
...
Element* List_innerproduct_eval::_eval(LispE* lisp) {
   ...
   if (!t1 || !t2 || t1 != t2)
      ...
      op1->release();
      op2->release();
      return lisp->check_error(this,  new Error("Error: arguments for '.' must be compatible lists or matrices"), idxinfo);
   }
   ...
   if (t1 == a_flat_list) {
      if (sx_1 != sx_2)
         ...
         op1->release();
         op2->release();
         return lisp->check_error(this,  new Error("Error: lists should have the same size for '.'"), idxinfo);
      }
   ...
   if (sy_1 != sx_2)
      ...
      op1->release();
      op2->release();
      return lisp->check_error(this,  new Error("Error: incompatible matrices"), idxinfo);
   }
   ...
   switch (first_element->type) {
      case t_matrix_string: {
         ...
         if (sz1.size() < sz2.size())
            ...
            second_element->release();
            first_element->release();
            return lisp->check_error(this,  new Error("Error: Dimension error"), idxinfo);
         }
...
Element* List_tensor_number_eval::_eval(LispE* lisp) {
   long sz = size();
   Element* e = zero_;
   vecte<long> shape;
   long s;
   ...
   if (sz == 2) {
      ...
      if (!e->isList())
         {
         e->release();
         return lisp->check_error(this,  new Error("Error: The first element should be a list"), idxinfo);
      }
      if (e->type >= t_tensor_string && e->type <= t_tensor_integer)
         ...
         if (!c->isNumber())
            {
            e->release();
            return lisp->check_error(this,  new Error("Error: this list should contain numbers"), idxinfo);
         }
      }
      Numbers l;
      e->flatten(lisp,&l);
...
Element* List_tensor_short_eval::_eval(LispE* lisp) {
   long sz = size();
   Element* e = zero_;
   vecte<long> shape;
   long s;
   ...
   if (sz == 2) {
      ...
      if (e->type >= t_tensor_string && e->type <= t_tensor_integer)
         ...
         if (!c->isNumber())
            {
            e->release();
            return lisp->check_error(this,  new Error("Error: this list should contain integers"), idxinfo);
         }
      }
      Shorts l;
      e->flatten(lisp,&l);
...
Element* List_tensor_string_eval::_eval(LispE* lisp) {
   long sz = size();
   Element* e = zero_;
   vecte<long> shape;
   long s;
   ...
   if (sz == 2) {
      ...
      if (e->type >= t_tensor_string && e->type <= t_tensor_integer)
         ...
         if (!c->isString())
            {
            e->release();
            return lisp->check_error(this,  new Error("Error: this list should contain strings"), idxinfo);
         }
      }
      Strings l;
      e->flatten(lisp,&l);
...
Element* List_tensor_stringbyte_eval::_eval(LispE* lisp) {
   long sz = size();
   Element* e = zero_;
   vecte<long> shape;
   long s;
...
Element* List_tensor_integer_eval::_eval(LispE* lisp) {
   long sz = size();
   Element* e = zero_;
   vecte<long> shape;
   long s;
...
Element* List_to_tensor_eval::_eval(LispE* lisp) {
   ...
   if (values->structuredList(0, shape, element_type)) {
      ...
      switch (element_type) {
         ...
         default:
            values->release();
         return new Error("Error: cannot apply to_tensor to this object");
      }
      switch (shape.size()) {
         case 1:
            values->release();
...
Element* List_matrix_string_eval::_eval(LispE* lisp) {
   long sz = size();
   Element* e = zero_;
   long sx, sy;
   ...
   if (sz == 2) {
      ...
      if (!e->isList() || !e->index(0)->isList() || !e->index(0)->index(0)->isNumber())
         {
         e->release();
         return lisp->check_error(this,  new Error("Error: Cannot initialize a matrix with this value"), idxinfo);
      }
      long size_x = e->size();
...
Element* List_matrix_stringbyte_eval::_eval(LispE* lisp) {
   long sz = size();
   Element* e = zero_;
   long sx, sy;
...
Element* List_matrix_number_eval::_eval(LispE* lisp) {
   long sz = size();
   Element* e = zero_;
   long sx, sy;
...
Element* List_matrix_short_eval::_eval(LispE* lisp) {
   long sz = size();
   Element* e = zero_;
   long sx, sy;
...
Element* List_matrix_integer_eval::_eval(LispE* lisp) {
   long sz = size();
   Element* e = zero_;
   long sx, sy;
...
Element* List_rho_eval::_eval(LispE* lisp) {
   ...
   if (e->isInteger()) {
      ...
      if (!e->isList() || listsize != 3)
         ...
         res->release();
         lisp->reset_to_true(sb);
         return lisp->check_error(this,  new Error("Error: Shape elements are either integers or a list of integers"), idxinfo);
      }
   ...
   if (!e->isList())
      ...
      res->release();
      lisp->reset_to_true(sb);
      return lisp->check_error(this,  new Error("Error: last argument should be a list"), idxinfo);
   }
   if (listsize == 3) {
      sz1 = shape[0];
      switch (e->type) {
...
Element* List_solve_eval::_eval(LispE* lisp) {
   ...
   if (element->type != Y->type)
      {
      element->release();
      return lisp->check_error(this,  new Error("Error: solve can only be applied to matrices of same type"), idxinfo);
   }
   ...
   case t_matrix_number:
      ...
      break;
         default:
            {
         element->release();
         return lisp->check_error(this,  new Error("Error: solve can only be applied to matrices"), idxinfo);
      }
   }
...
Element* List_ludcmp_eval::_eval(LispE* lisp) {
   ...
   case t_matrix_number:
      ...
      break;
         default:
            element->release();
         return new Error("Error: solve can only be applied to matrices");
   }
   element->release();
   return res;
}
...
Element* List_lubksb_eval::_eval(LispE* lisp) {
   ...
   if (idxs->type != t_integers)
      ...
      return lisp->check_error(this,  new Error("Error: the second element should be an integers_ (a list of integers)"), idxinfo);
   }
      ...
      case t_matrix_number:
         ...
         break;
            default:
               {
            element->release();
            return lisp->check_error(this,  new Error("Error: inversion can only be applied to matrices"), idxinfo);
         }
      }
   }
   element->release();
