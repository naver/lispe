...
void setrules(vecte_n<u_ustring>& v) {
   if (main && lisp->id_thread)
      {
      lisp->delegation->set_error(new Error("You cannot modify the main tokenizer rules in a thread"));
      return;
   }
...
Element* setoperators(Set_s* lst) {
   if (main && lisp->id_thread)
      return new Error("You cannot modify the main tokenizer oeprators in a thread");
...
Element* methodBase(LispE* lisp) {
   ...
   if (b <= 1)
      return new Error("Error: cannot convert to this base");
   ...
   for (n = 0; n < w.size(); n++) {
      ...
      if (!mcaracs.count(wc) || mcaracs[wc] >= b)
         return new Error(U"Error: Cannot analyze this string in this base.");
      v *= b;
      v += mcaracs[wc];
   }
...
Element* _eval(LispE* lisp) {
   ...
   case str_getstruct:
      ...
      case str_set_rules: {
         ...
         if (!lst->isList())
            return new Error("Error: This function expects a list");
      ...
      case str_set_operators: {
         ...
         if (lst->type != t_sets)
            return new Error("Error: This function expects a set of strings (sets)");
