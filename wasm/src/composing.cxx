...
Element* List::evall_repeat_cps(LispE* lisp) {
   ...
   if (!lisp->composition_stack.empty())
      return new Error("Error: cannot apply 'repeat' with a context");
...
Element* List::evall_map_cps(LispE* lisp) {
   ...
   if (lisp->composition_stack.empty()) {
      if (size() < 3)
         return new Error("Error: Wrong number of arguments for 'map'");
...
Element* List::evall_filter_cps(LispE* lisp) {
   ...
   if (lisp->composition_stack.empty()) {
      if (size() < 3)
         return new Error("Error: Wrong number of arguments for 'filter'");
      lisp->composition_stack.push_back(liste[2]);
   }
   else
      if (size() < 2)
...
Element* List::evall_take_cps(LispE* lisp) {
   ...
   if (lisp->composition_stack.empty()) {
      if (size() < 3)
         return new Error("Error: Wrong number of arguments for 'take'");
      lisp->composition_stack.push_back(liste[2]);
   }
   else
      if (size() < 2)
...
Element* List::evall_drop_cps(LispE* lisp) {
   ...
   if (lisp->composition_stack.empty()) {
      if (size() < 3)
         return new Error("Error: Wrong number of arguments for 'drop'");
      lisp->composition_stack.push_back(liste[2]);
   }
   else
      if (size() < 2)
...
Element* List::evall_takewhile_cps(LispE* lisp) {
   ...
   if (lisp->composition_stack.empty()) {
      if (size() < 3)
         return new Error("Error: Wrong number of arguments for 'takewhile'");
      lisp->composition_stack.push_back(liste[2]);
   }
   else
      if (size() < 2)
...
Element* List::evall_dropwhile_cps(LispE* lisp) {
   ...
   if (lisp->composition_stack.empty()) {
      if (size() < 3)
         return new Error("Error: Wrong number of arguments for 'dropwhile'");
      lisp->composition_stack.push_back(liste[2]);
   }
   else
      if (size() < 2)
...
Element* List::evall_for_cps(LispE* lisp) {
   if (size() < 4)
      return new Error("Error: Wrong number of arguments for 'for'");
   if (lisp->composition_stack.empty())
      lisp->composition_stack.push_back(liste[2]);
...
Element* List::evall_foldl_cps(LispE* lisp) {
   ...
   if (lisp->composition_stack.empty()) {
      if (size() < 4)
         return new Error("Error: Wrong number of arguments for 'foldl'");
...
Element* List::evall_scanl_cps(LispE* lisp) {
   ...
   if (lisp->composition_stack.empty()) {
      if (size() < 4)
         return new Error("Error: Wrong number of arguments for 'scanl'");
      lisp->composition_stack.push_back(liste[3]);
   }
   else
      if (size() < 3)
...
Element* List::evall_foldr_cps(LispE* lisp) {
   ...
   if (lisp->composition_stack.empty()) {
      if (size() < 4)
         return new Error("Error: Wrong number of arguments for 'foldr'");
      lisp->composition_stack.push_back(liste[3]);
   }
   else
      if (size() < 3)
...
Element* List::evall_scanr_cps(LispE* lisp) {
   ...
   if (lisp->composition_stack.empty()) {
      if (size() < 4)
         return new Error("Error: Wrong number of arguments for 'scanr'");
      lisp->composition_stack.push_back(liste[3]);
   }
   else
      if (size() < 3)
...
Element* List::evall_foldl1_cps(LispE* lisp) {
   ...
   if (lisp->composition_stack.empty()) {
      if (size() < 3)
         return new Error("Error: Wrong number of arguments for 'foldl1'");
      lisp->composition_stack.push_back(liste[2]);
   }
   else
      if (size() < 2)
...
Element* List::evall_scanl1_cps(LispE* lisp) {
   ...
   if (lisp->composition_stack.empty()) {
      if (size() < 3)
         return new Error("Error: Wrong number of arguments for 'scanl1'");
      lisp->composition_stack.push_back(liste[2]);
   }
   else
      if (size() < 2)
...
Element* List::evall_foldr1_cps(LispE* lisp) {
   ...
   if (lisp->composition_stack.empty()) {
      if (size() < 3)
         return new Error("Error: Wrong number of arguments for 'foldr1'");
      lisp->composition_stack.push_back(liste[2]);
   }
   else
      if (size() < 2)
...
Element* List::evall_scanr1_cps(LispE* lisp) {
   if (lisp->composition_stack.empty()) {
      if (size() < 3)
         return new Error("Error: Wrong number of arguments for 'scanr1'");
      lisp->composition_stack.push_back(liste[2]);
   }
   else
      if (size() < 2)
...
void List_switch_eval::build(LispE* lisp) {
   ...
   for (long i = 2; i < size(); i++) {
      ...
      if (e->type != t_list || !e->size())
         {
         lisp->delegation->set_error(new Error("Error: wrong 'switch statement'"));
         return;
      }
      ...
      if (v == true_)
         ...
         if (v->isString() || v->isNumber()) {
            ...
            {
            lisp->delegation->set_error(new Error("Error: Unknown statement"));
            return;
         }
      }
   }
      ...
      if (liste[0]->isExecutable(lisp)) {
         ...
         if (element->isArgumentFunction())
            ...
            if (!element->isAtom()) {
               ...
               if (!element->isAtom())
                  return new Error("Error: Missing argument in defpat function");
            }
