...
Element* Element::matrix_product(LispE* lisp, Element* m2, long, long, long) {
   return new Error("Error: '+*' only apply to numerical vectors");
}
...
Element* Element::eval_lambda_min(LispE*) {
   return new Error("Error: this is not a lambda function");
}


//------------------------------------------------------------------------------------------
Element* Element::minimum(LispE* lisp) {
   return new Error("Error: cannot find the minimum for this object");
}


//------------------------------------------------------------------------------------------
Element* Element::maximum(LispE* lisp) {
   return new Error("Error: cannot find the maximum for this object");
}


//------------------------------------------------------------------------------------------
Element* Element::minmax(LispE* lisp) {
   return new Error("Error: cannot find the minmax for this object");
}
...
Element* Element::invert_sign(LispE* lisp) {
   return new Error("Error: Cannot invert this object");
}
...
Element* Dictionary_as_list::dictionary(LispE* lisp) {
   ...
   if (!choice || keyvalues.size() != valuevalues.size()) {
      if (valuevalues.size() == 0) {
         ...
         return new Error("Error: dictionary has a different number of key/value");
      }
   }
   else {
      if (type == t_float || type == t_number)
...
Element* Element::rank(LispE* lisp, vecte<long>& positions) {
   return new Error("Error: cannot apply 'rank' to this element");
}
...
Element* Infiniterangenumber::loop(LispE* lisp, int16_t label, List* code) {
   if (!increment)
      return new Error("Error: increment cannot be 0");
   long i_loop;
   Element* e = null_;
   long sz = code->liste.size();
...
Element* Cyclelist::loop(LispE* lisp, int16_t label, List* code) {
   ...
   if (element->type != t_list) {
      element->release();
      return new Error("Error: we can only cycle on list");
   }
...
Element* Element::insert_with_compare(LispE*, Element* e, List& comparison) {
   return new Error("Error: insertion impossible");
}
...
Element* Element::protected_index(LispE* lisp,Element*) {
   return new Error("Error: value cannot be access through index");
}
...
Element* Element::join_in_list(LispE* lisp, u_ustring& sep) {
   return new Error("Error: 'join' can only be used for lists");
}
...
Element* Element::replace(LispE* lisp, long i, Element* e) {
   return new Error("Error: cannot modify this element");
}
...
Element* Complexe::compare(LispE* lisp, Element* e) {
   if (e->type == t_complex) {
      ...
      return new Error("Error: cannot compare these values");
}
...
Element* Element::plus(LispE* lisp, Element* e) {
   return new Error("Error: cannot evaluate '+' with these operands");
}


Element* Element::bit_not(LispE* lisp) {
   return new Error("Error: cannot evaluate '~' for this operand");
}
...
Element* String::car(LispE* lisp) {
   if (content.size() == 0)
      return new Error("Error: Empty string");
...
Element* String::cadr(LispE* lisp, u_ustring& action) {
   ...
   for (i = action.size() - 1; i>= 0; i--) {
      if (action[i] == 'a') {
         if (i)
            return new Error("Error: cannot apply 'car/cdr' to one character");
         u = content[pos];
