...
Element* range(LispE* lisp, double init, double limit, double inc, bool int_eger) {
   ...
   return new Error("Error: Exceeding range");
}
...
void Matrice::concatenate(LispE* lisp, Element* e) {
   if (e->isList()) {
      if (e->size() != size_x)
         {
         lisp->delegation->set_error(new Error("Error: Length error"));
         return;
      }
      for (long i = 0; i < size_x; i++) {
...
void Tenseur::concatenate(LispE* lisp, Element* e) {
   if (e->isList()) {
      ...
      for (long i = 0; i < sz.size()-1; i++) {
         if (sz[i] != shape[i])
            {
            lisp->delegation->set_error(new Error("Error: Incompatible dimensions"));
            return;
         }
      }
   }
...
Element* Matrice::inversion(LispE* lisp) {
   if (size_x != size_y)
      return new Error("Error: we can only apply 'invert' to square matrices");
...
Element* Matrice::solve(LispE* lisp, Matrice* y) {
   if (size_x != size_y || y->size_x != y->size_y || size_x != y->size_x)
      return new Error("Error: we can only apply 'solve' to square matrices of equal sizes");
...
Element* Matrice::ludcmp(LispE* lisp) {
   if (size_x != size_y)
      return new Error("Error: we can only apply 'ludcmp' to square matrices");
...
Element* Matrice::lubksb(LispE* lisp, Integers* idxs, Matrice* Y) {
   if (size_x != size_y || idxs->size() != size_x) {
      return new Error("Error: we can only apply 'lubksb' to square matrices with the same number of indexes");
   }
   ...
   if (Y == NULL) {
      ...
      if (Y->size_x != size_x)
         return new Error("Error: we can only apply 'lubksb' to square matrices of the same shape");
   }
...
float Element::checkFloat(LispE* lisp) {
   ...
   s += L"'";
   {
      lisp->delegation->set_error(new Error(s));
      return 0;
   }
}
...
Element* List_dividen::_eval(LispE* lisp) {
   Element* first_element = liste[1]->eval(lisp);
   first_element = first_element->copyatom(lisp, 1);
...
Element* List_divide2::_eval(LispE* lisp) {
   Element* first_element = liste[1]->eval(lisp);
   if (!first_element->isList())
      return new Error("Error: cannot apply '/' to one element");
...
Element* List_divide3::_eval(LispE* lisp) {
...
Element* List_minusn::_eval(LispE* lisp) {
   Element* first_element = liste[1]->eval(lisp);
   first_element = first_element->copyatom(lisp, 1);
...
Element* List_minus2::_eval(LispE* lisp) {
   Element* first_element = liste[1]->eval(lisp);
   if (!first_element->isList())
      return new Error("Error: cannot apply '-' to one element");
...
Element* List_minus3::_eval(LispE* lisp) {
   Element* first_element = liste[1]->eval(lisp)->copyatom(lisp, 1);
...
Element* List_multiplyn::_eval(LispE* lisp) {
   Element* first_element = liste[1]->eval(lisp);
   first_element = first_element->copyatom(lisp, 1);
...
Element* List_multiply2::_eval(LispE* lisp) {
   Element* first_element = liste[1]->eval(lisp);
   if (!first_element->isList())
      return new Error("Error: cannot apply '*' to one element");
...
Element* List_multiply3::_eval(LispE* lisp) {
   Element* first_element = liste[1]->eval(lisp)->copyatom(lisp, 1);
...
Element* List_plusn::_eval(LispE* lisp) {
   Element* first_element = liste[1]->eval(lisp);
   first_element = first_element->copyatom(lisp, 1);
...
Element* List_plus2::_eval(LispE* lisp) {
   Element* first_element = liste[1]->eval(lisp);
   if (!first_element->isList())
      return new Error("Error: cannot apply '+' to one element");
...
Element* List_plus3::_eval(LispE* lisp) {
   Element* first_element = liste[1]->eval(lisp)->copyatom(lisp, 1);
...
Element* List_power2::_eval(LispE* lisp) {
   ...
   if (label < l_final) {
      ...
      if (liste[1]->isList() && liste[1]->index(0)->label() == l_at) {
         if (liste[1]->index(1)->label() < l_final)
            return new Error("Error: Expecting a variable in embedded '@'");
         ...
         return new Error("Error: Missing variable");
   }
...
Element* List_divideequal_list::_eval(LispE* lisp) {
   long i;
   int16_t listsize;
   Element* first_element = liste[1];
...
Element* List_divideequal_var::_eval(LispE* lisp) {
   Element* first_element = liste[1];
   Element* second_element = null_;
   Element* lst = this;
...
Element* List_minusequal_list::_eval(LispE* lisp) {
   long i;
   int16_t listsize;
   Element* first_element = liste[1];
...
Element* List_minusequal_var::_eval(LispE* lisp) {
   Element* first_element = liste[1];
   Element* second_element = null_;
   Element* lst = this;
...
Element* List_multiplyequal_list::_eval(LispE* lisp) {
   long i;
   int16_t listsize;
   Element* first_element = liste[1];
...
Element* List_multiplyequal_var::_eval(LispE* lisp) {
   Element* first_element = liste[1];
   Element* second_element = null_;
   Element* lst = this;
...
Element* List_plusequal_list::_eval(LispE* lisp) {
   long i;
   int16_t listsize;
   Element* first_element = liste[1];
...
Element* List_plusequal_var::_eval(LispE* lisp) {
   Element* first_element = liste[1];
   Element* second_element = null_;
   Element* lst = this;
...
Element* complex_math(Complex* c) {
   ...
   switch (m) {
      ...
      default:
         c->release();
      return new Error("Error: Cannot apply this method on a complex value.");
   }
}
