...
Element* Concept::concept_remove(Concept* c) {
   if (c == this) {
      ...
      if (c->ontologie != ontologie)
         return new Error("Error: these concepts do not belong to the same ontology");
...
Element* Concept::bit_and(LispE* lisp, Element* e) {
   if (e->type != type)
      return new Error("Error: cannot apply '&' to this operand");
...
Element* Ontology::find(LispE* lisp, Concept* c) {
   if (c->ontologie != this)
      return new Error("Error: this concept does not belong to this ontology");
   u_ustring w;
...
Element* Ontology::find(LispE* lisp, u_ustring& w) {
   ...
   if (a == indexes.end())
      return new Error("Error: unknown concept");
   return a->second;
}
   ...
   switch (reg) {
      ...
      case ontology_concept: {
         ...
         if (e->type != l_ontology)
            return new Error("Error: the first argument should be an ontology");
      ...
      case ontology_create: {
         ...
         if (conc != null_) {
            if (conc->type != l_concept)
               return new Error("Error: the last element should be a 'concept'");
      ...
      case ontology_contain: {
         ...
         if (conc->type != l_concept || large_conc->type != l_concept)
            return new Error("Error: We can only compare concepts");
      ...
      case ontology_add: {
         ...
         if (conc->type != l_concept)
            return new Error("Error: We can only enrich with concepts");
      ...
      case ontology_remove: {
         ...
         Element* add = lisp->get(U"a");
         if (conc->type != l_concept)
            return new Error("Error: We can only remove a concept from another concept");
         if (add->type == l_concept)
      ...
      case ontology_absurdp: {
         Element* conc = lisp->get(U"conc");
         if (conc->type != l_concept)
            return new Error("Error: the first argument should be a concept");
