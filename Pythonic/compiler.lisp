...
(setq star_func0 `(defun %1 (tokens %4 vp)
   (setq v ())
   (while %2
      (nconc v %3)
   )
            ...
            (check functions
               (loop f functions
                  ...
                  (switch (@ (string (car f)) 0)
                     ...
                     ("S" (block
                           (if (eq (car tst) 'or)
                              (setq func (format star_func (car f) tst val idx))
                              (setq func (format star_func0 (car f) tst val idx)))))
                  )         
                  (+= the_code (prettify (car (to_list func)) 100) "\n")
               )
               (setq functions ())
            )
         )
...
(+= the_code func)
(fwrite (+ _current grammar ".lisp") (prettify the_code 100))
